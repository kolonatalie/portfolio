...
### GSAP
- **Control**  
Full Control over timing, direction, and flow.
- **Sequencing**  
Easy Timeline. Effortlessly layer and sync multiple animations.
- **Use Case**  
Complex sequences, dynamic interactions, and scroll-linked effects (`ScrollTrigger`).

### CSS
- **Control**  
Limited. Once started, it's hard to stop or adjust precisely.
- **Sequencing**  
Difficult. Requires manual, tedious timing with `delay` and percentage keyframes.
- **Use Case**  
Simple hovers, quick fades, and loading spinners.
...
> **The CPU's Worst Nightmare:**  
Operations that force it to recalculate the page layout.  

  *

### **GPU** (_Graphics Processing Unit_)
...
> **The GPU's Best Friend:**  
Operations like 3D rendering (WebGL) and combining pre-painted layers.
...
1. **Layout (Reflow)**.  
The browser calculates the size and position of every element.  
 ***This is CPU-heavy.***

2. **Paint**.  
The browser draws the colors, shadows, and text onto distinct layers.  
***This is also CPU-heavy.***

3. **Compositing**.  
The layers are merged together and displayed.  
***This is GPU-accelerated and fast.***
...
> The golden rule:  
**Animate only properties that skip Layout and Paint.**


### **Avoid for Animation.** The CPU Bottlenecks 
...
- **Triggers Layout** 
    ...
    `padding`,  
    `border`,  
    `top`/`left`(on non-absolute elements),  
    `font-size`.
...
> P.S.: I’m actively looking for a Junior Frontend Developer role where I can apply my Visual Engineering and clean code principles to create beautifully performing user interfaces. [Let's connect](/#contact)
